...
function Pricing_Section() {
  return (
    <section id="pricing" className="py-8 sm:py-12 md:py-24 lg:py-32 bg-secondary dark:bg-gray-900">
      <div className="min-w-full container px-4 sm:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          <div className="space-y-2">
            ...
            <p className="max-w-[900px] text-sm text-muted-foreground sm:text-base md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed dark:text-gray-300">
              Choose the plan that's right for you, whether you're just starting out or looking for more advanced
              features.
            </p>
          </div>
        </div>
        <div className="mx-auto grid max-w-5xl gap-6 py-12 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <Card 
              key={`course-${index}`} 
              className={`${course.popular ? "border-primary shadow-lg dark:shadow-primary/20" : ""} dark:bg-gray-800 dark:border-gray-700`}
            >
              <CardHeader className="text-center">
                <CardTitle className="dark:text-white">{course.title}</CardTitle>
                <div className="flex items-baseline justify-center">
                  <span className="text-3xl font-bold dark:text-white">{course.price}</span>
                  <span className="text-sm text-muted-foreground ml-1 dark:text-gray-300">per course</span>
                </div>
